...
//		click(signIn);
	...
	public void SetUpCEUsertwoSSO(String ReferenceLogin) throws IOException, InterruptedException {
		...
		else if (Url.contains("https://truecoverage-uat.benefitalign.com/mme/")) {
		    ExtentSuccessMessage("Logged in Environment: UAT");
		    System.out.println("Logged in Environment: UAT");
		}
		...
		for (String key : configloader().stringPropertyNames()) {
			...
			if (key.contains(ReferenceLogin) && Url.contains("https://ce-uat.benefitalign.com/")) {
                ...
                String KeyValue = key + ";" + value;
                String replaceKeyValue = KeyValue.replace(ReferenceLogin + "_", "");
                if (replaceKeyValue.contains("UserNameSSOUAT1")) {
                       UserName = replaceKeyValue;
                       String[] arrOfUserName = UserName.split(";");
                       UserName = arrOfUserName[1];
                }
                if (replaceKeyValue.contains("PasswordSSOUAT1")) {
                       Password = replaceKeyValue;
                       String[] arrOfPassword = Password.split(";");
                       Password = arrOfPassword[1];
                }
			...
			else if (key.contains(ReferenceLogin) && Url.contains("https://truecoverage-pp.brokerengage.net/")) {
				String KeyValue = key + ";" + value;
				String replaceKeyValue = KeyValue.replace(ReferenceLogin + "_", "");
			...
			else if (key.contains(ReferenceLogin) && Url.contains("https://ce-dev.benefitalign.com/")) {
				String KeyValue = key + ";" + value;
				String replaceKeyValue = KeyValue.replace(ReferenceLogin + "_", "");
				if (replaceKeyValue.contains("UserName2")) {
					UserName = replaceKeyValue;
		...
		if(Url.contains("https://truecoverage-qa.benefitalign.com/mme/") || Url.contains("https://truecoverage-pp.brokerengage.net/"))
		 {
			...
			click(SSO_signin);
			click(YesButton);
        if(Url.contains("https://truecoverage-qa.benefitalign.com/mme/") || Url.contains("https://truecoverage-uat.benefitalign.com/mme/"))
        {
            input(SSO_userId, UserName);
               click(SSO_next);
               input(SSO_Password, Password);
	...
	public void SetUpCEUseroneCognito(String ReferenceLogin) throws IOException, InterruptedException {
		String Browser = configloader().getProperty("Browser");
		String Url = configloader().getProperty("URL");
		ExtentSuccessMessage("URL: "  + Url);
		String jenkinsBrowser = System.getProperty("BROWSER");
		...
		if (Url.contains("https://mme.brokerengage.net/mme/")) {
		    ExtentSuccessMessage("Logged in Environment: AWS PP");
		    System.out.println("Logged in Environment: AWS PP cognito");
		} else if (Url.contains("https://ce-uat.benefitalign.com/"))
		{
		    ExtentSuccessMessage("Logged in Environment: UAT");
		    System.out.println("Logged in Environment: UAT");
	...
	public void SetUpCEUseroneSSO(String ReferenceLogin) throws IOException, InterruptedException {
	...
	public void SetUpCEUseroneHeadless(String ReferenceLogin) throws IOException, InterruptedException {
